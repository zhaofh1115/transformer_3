...
class MultiHeadAttention(nn.Module):
    ...
    def pad(self, Q, K, V, mask, chunk_size: int):

        # Compute Overflows
        overflow_Q = Q.size(1) % chunk_size
        overflow_KV = K.size(1) % chunk_size
        
        padding_Q = chunk_size - overflow_Q if overflow_Q else overflow_Q
        padding_KV = chunk_size - overflow_KV if overflow_KV else overflow_Q
        ...
        if mask is not None:
            ...
            # (B, 1, 1, T) -> (B, 1, 1, T + P) 
            if mask.size(2) == 1:
                mask = F.pad(mask, pad=[0, padding_KV], value=1.0)
            # (B, 1, T, T) -> (B, 1, T + P, T + P)
            else:
                mask = F.pad(mask, pad=[0, padding_Q, 0, padding_KV], value=1.0)
...
class RelPosMultiHeadSelfAttention(MultiHeadAttention):
    ...
    def __init__(self, dim_model, num_heads, causal, max_pos_encoding):
        ...
        self.u = nn.Parameter(torch.Tensor(self.num_heads, self.dim_head)) # Content bias
        self.v = nn.Parameter(torch.Tensor(self.num_heads, self.dim_head)) # Pos bias
        torch.nn.init.xavier_uniform_(self.u) # glorot uniform
        torch.nn.init.xavier_uniform_(self.v) # glorot uniform
    ...
    def forward(self, Q, K, V, mask: torch.Tensor, att_cache: torch.Tensor):
        ...
        if att_cache.size(0) > 0:    
            key_cache, value_cache = torch.split(
                att_cache, att_cache.size(-1) // 2, dim=-1)
...
class GroupedRelativeSinusoidalPositionalEncoding(nn.Module):
    ...
    def forward(self, batch_size: int, seq_len: torch.Tensor, hidden_len: torch.Tensor,):
