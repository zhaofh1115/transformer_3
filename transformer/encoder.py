...
# efficientconformer subsampling
from wenet.transformer.efficientsubsampling import (
    Conv1dSubsampling,
    Conv2dSubsampling,
...
from wenet.transformer.efficientconformer_attention import SinusoidalPositionalEncoding
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        """
        # T = xs.size(1)
        # masks = ~make_pad_mask(xs_lens, T).unsqueeze(1)  # (B, 1, T) padding
        # xs = self.global_cmvn(xs)
        # if self.conformer_type == "efficientconformer":
        ...
        #         xs, masks, _, _ = layer(xs, chunk_masks)

        # else:
        #     xs, pos_emb, masks = self.embed(xs, masks) #subsampling and rel_pos
        ...
        # Subsampling Module
        x, x_len, masks = self.subsampling_module(x, x_len, mask)
        ...
        # EfficientConformer layers
        for layer in self.encoders:
            x, _, _, _ = layer(x, mask)
            ...
            if layer.stride > 1:
                ...
                if x_len is not None:
                    x_len = torch.div(x_len - 1, layer.stride, rounding_mode='floor') + 1
...
class EfficientConformerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        **params,
        ...
        # Conformer Blocks
        self.encoders = torch.nn.ModuleList([EfficientConformerEncoderLayer(
        ...
        ) for block_id in range(params["num_blocks_efficientconformerencoder"])])